<script lang="ts">
	...
	import {
		generateLogToPixelIndexMap,
		isSpectrumType,
		spectrumTypes,
		type LogToPixelIndexMap,
		type SpectrumType,
		capitalize,
		clamp
	} from '$lib';
...
<main>
	<div class="container">
		...
		<section>
			...
			<p>
				<label
					>{$_('parameters_fft_size')}
					<select
						name="spectrum-fft-size"
						bind:value={fftSize}
						on:change={(e) => {
					...
					>
						{#each [32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768] as size}
							<option value={size} selected={size == fftSize} disabled={size < playerWidth}
								>{capitalize(String(size))}</option
							>
						{/each}
					</select>
				</label>
					...
					<p>
						<label
							...
							<input
								...
								on:change={(e) =>
									(spectrumSensitivity = clamp(Number(e.currentTarget.value), 0.1, 10))}
							/>
						</label>
					</p>
					<p>
						<label
							>{$_('parameters_grid_size')}
							<input
								type="number"
								min="0"
		...
		<PlayerMenu {playerRef} {audioContext} {audioRef} {imageLoaded} {recorderDestination} />
